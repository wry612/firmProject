...
for (var i = 0; i < 240; i++) {
    ...
    num = num + Math.random() * 20-10;
    lineData.push(num.toFixed());
}
lineOption = {
    backgroundColor:'#fff',
    tooltip: {
...
KOption = {
    ...
    tooltip: {
        ...
        formatter: function (params) {
            params = params[0];
            var date = params.name;
            return date + '<br>' +'开盘：'+params.value[0].toFixed(2)+'  最高：'+params.value[1].toFixed(2)+ '<br>'+'收盘：'+params.value[3].toFixed(2)+'  最低：'+params.value[2].toFixed(2);
        },
        textStyle: {
            fontSize: fontSize,
            color: '#fff'
